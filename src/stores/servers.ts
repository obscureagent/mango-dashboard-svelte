...
function createServerStore() {
	...
	return {
		...
		fetchServers: async () => {
			if (!token) return;
			const response = await fetch('http://localhost:5174/mock/servers', {
		...
		getServer: async (id: string): Promise<Server | null> => {
			...
			const response = await fetch(`http://localhost:5174/mock/servers/${id}`, {
				headers: { Authorization: `Bearer ${token}` }
			});
		...
		startServer: async (id: string) => {
			if (!token) return;
			await fetch(`http://localhost:5174/mock/servers/${id}/start`, {
				method: 'POST',
				headers: { Authorization: `Bearer ${token}` }
			});
			update(servers =>
		...
		stopServer: async (id: string) => {
			if (!token) return;
			await fetch(`http://localhost:5174/mock/servers/${id}/stop`, {
				method: 'POST',
				headers: { Authorization: `Bearer ${token}` }
			});
			update(servers =>
		...
		restartServer: async (id: string) => {
			if (!token) return;
			await fetch(`http://localhost:5174/mock/servers/${id}/restart`, {
				method: 'POST',
				headers: { Authorization: `Bearer ${token}` }
			});
			update(servers =>
		...
		deleteServer: async (id: string) => {
			if (!token) return;
			await fetch(`http://localhost:5174/mock/servers/${id}`, {
				method: 'DELETE',
				headers: { Authorization: `Bearer ${token}` }
			});
